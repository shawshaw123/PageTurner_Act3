...
class FictionBooksSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (!$fictionCategory) {
            $this->command->error('Fiction category not found!');
            return;
        }
        ...
        foreach ($fictionBooks as $bookData) {
            $book = Book::create(array_merge($bookData, [
                'category_id' => $fictionCategory->id,
            ]));
            $this->command->info("Created: {$book->title} by {$book->author}");
        }
    }
}
