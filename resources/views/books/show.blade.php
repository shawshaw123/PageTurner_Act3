...
<div class="bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden">
    <div class="md:flex">
        ...
        <div class="md:w-1/3 bg-gray-50 p-10 flex items-center justify-center border-b md:border-b-0 md:border-r border-gray-100">
            @if($book->cover_image)
                <img src="{{ asset('storage/' . $book->cover_image) }}" alt="{{ $book->title }}" class="max-h-[500px] shadow-2xl rounded-lg object-contain">
            @else
                <div class="bg-gray-200 h-64 w-48 rounded-lg flex items-center justify-center shadow-inner">
            ...
            <div class="mt-10">
                @auth
                    <form action="{{ route('orders.store') }}" method="POST" class="flex flex-col sm:flex-row gap-4">
                        ...
                        <button type="submit" @disabled($book->stock_quantity <= 0) 
                            class="flex-grow bg-brand-darkgreen text-white px-8 py-4 rounded-lg font-bold text-lg hover:bg-brand-amber hover:text-brand-darkgreen transition-all transform hover:-translate-y-1 shadow-lg disabled:bg-gray-400 disabled:transform-none">
                            {{ $book->stock_quantity > 0 ? 'BUY NOW' : 'OUT OF STOCK' }}
                        </button>
                    </form>
                @else
